...
class Integration extends Component {
  ...
  getFilteredData = (data) => {
    ...
    const startDate1 = moment(startDate).format('DD/MM/YYYY'); 
    const endDate1 = moment(endDate).format('DD/MM/YYYY');
    const startDateMoment = moment(startDate1,'DD/MM/YYYY');
    const endDateMoment = moment(endDate1,'DD/MM/YYYY');
    if (startDate && endDate) {
      filteredData = data.filter((row) => {
        const issuedDateMoment = moment(row.issuedDate, 'DD/MM/YYYY');
        const dueDateMoment = moment(row.dueDate, 'DD/MM/YYYY');
        return (issuedDateMoment.isSameOrAfter(startDateMoment, 'day') &&
        dueDateMoment.isSameOrBefore(endDateMoment, 'day'));
      });
    }
  ...
  render() {
    ...
    const { selectedCustomers } = this.state;
    const filteredCanceledData = this.getFilteredData(combinedData.filter((item) => item.paymentStatus === 'CANCELED'));
    const filteredPaidData = this.getFilteredData(combinedData.filter((item) => item.paymentStatus === 'PAID'));
    const filteredPendingData = this.getFilteredData(combinedData.filter((item) => item.paymentStatus === 'PENDING'));
